...
public class NetworkManager : MonoBehaviour {
	...
	private void OnPlayerConnected(NetworkPlayer player) {
		...
		networkView.RPC("UpdatePlayerConnectionStatus", RPCMode.AllBuffered, 1);
	}

	private void OnPlayerDisconnected(NetworkPlayer player) {
		Debug.Log("I'm Disconnected! (Called from the SERVER!");
		networkView.RPC("UpdatePlayerConnectionStatus", RPCMode.All, -1);
	}

	private void OnConnectedToServer() {
		Debug.Log("I'm Connected! (Called from the CLIENT)");
		this.gameManager = GameManager.Waiting;
		networkView.RPC("AddPlayer", RPCMode.Server, Network.player, this.tmpName);
	}
		...
		foreach (PlayerInformation element in playerInfo) {
			if (element.tank == null) {
				...
				if (element.spawnTime < 1.0f) {
					networkView.RPC("RespawnPlayer", RPCMode.All, element.playerID, i);
				}
			}
			i++;
		}
	...
	private void AddPlayer(NetworkPlayer player, string playerName) {
		...
		foreach (PlayerInformation element in playerInfo) {
			networkView.RPC("UpdatePlayerList", RPCMode.Others, element.playerID, element.name);
		}
	}
	...
	private void StartGame() {
		...
		networkView.RPC("SpawnPlayers", RPCMode.All, Network.player);
	}
	...
	private void SpawnPlayers(NetworkPlayer player) {
		if (Network.isServer) {
			...
			networkView.RPC("UpdatePlayerNames", RPCMode.Others);
		}
	}
	...
	private void RespawnPlayer(NetworkPlayer playerID, int indx) {
		if (Network.isServer) {
			...
			networkView.RPC("UpdateRespawnPlayer", RPCMode.Others, playerID, indx);
		}
	}
	...
	public void AddKills(NetworkPlayer id) {
		...
		networkView.RPC("UpdateKills", RPCMode.Others, id, tmpKills);
	}
	...
	public void AddDeaths(NetworkPlayer id) {
		...
		networkView.RPC("UpdateDeaths", RPCMode.Others, id, tmpDeaths);
	}
	...
	public void AddSpawnTime(NetworkPlayer id) {
		...
		networkView.RPC("UpdateSpawnTime", RPCMode.Others, id, tmpSpawnTime);
	}
