...
public class PlayerController : MonoBehaviour {
	...
	private void Update() {
		...
		if (!this.isHorizontal) {
			if (Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.UpArrow)) {
				if (transform.position.z < 9.0f) {
					...
					if (Network.isServer) networkView.RPC("UpdateUserInput", RPCMode.All, this.playerRotation, this.moveDirection, this.isVertical, this.isHorizontal, (int)this.playerScreenPosition);
					if (Network.isClient) networkView.RPC("SendUserInput", RPCMode.Server, this.playerRotation, this.moveDirection, this.isVertical, this.isHorizontal, (int)this.playerScreenPosition);
		...
		if (Input.GetKeyDown(KeyCode.Space) && fireIsReady) {
			...
			networkView.RPC("FireBullet", RPCMode.All, Network.player);
		}
	}
	...
	private void SendUserInput(Vector3 rotation, Vector3 moveDir, bool vert, bool horiz, int screenPos) {
		...
		networkView.RPC("UpdateUserInput", RPCMode.Others, rotation, moveDir, vert, horiz, screenPos);
	}
